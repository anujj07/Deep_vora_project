...
const routeLabel = (path: string) => path === '/work' ? 'WORK' : path === '/services' ? 'SERVICES' : path === '/about' ? 'ABOUT' : path === '/contact' ? 'CONTACT' : null;
...
export const Navigation: React.FC<NavigationProps> = ({ onNavigate, theme, onToggleTheme }) => {
  ...
  return (
    <>
      <header className={`fixed inset-x-0 top-0 z-[70] flex items-center justify-between border-b border-white/15 bg-[#161616]/95 px-5 text-[#fcfbf9] transition-[padding,background-color] duration-300 sm:px-8 md:px-12 lg:px-20 ${isCompact ? 'py-3 md:py-4' : 'py-5 md:py-7'}`}>
        <div className="flex min-w-0 items-center gap-3 font-mono-tech text-[10px] uppercase tracking-[0.14em] sm:text-[11px]">
          ...
          {breadcrumb.length > 0 && <span className="hidden min-w-0 items-center gap-2 text-white/55 md:flex" aria-label={`Current page: Deep Vora, ${breadcrumb.join(', ')}`}>{breadcrumb.map((item) => <React.Fragment key={item}><span>/</span><span className="truncate">{item}</span></React.Fragment>)}</span>}
        </div>
